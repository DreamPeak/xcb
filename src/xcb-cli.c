...
int main(int argc, char **argv) {
	...
	if (execute) {
		...
		if (argc > 0) {
			...
			cmd = dstr_cat(cmd, "\r\n");
			net_try_write(tcpsock, cmd, dstr_length(cmd), 10, NET_NONBLOCK);
			/* dstr_free(cmd); */
		...
		fprintf(stdout, "XCUBE CLI, Copyright (c) 2013-2017, "
		...
		while (loop) {
			...
			if (*line) {
				...
				if (tcpsock == -1)
					...
					net_try_write(tcpsock, line, strlen(line), 10, NET_NONBLOCK);
